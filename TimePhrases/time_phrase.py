...
def next_round():
	...
	option = randint(0, 1)
	# secondPhrase = option == 0 ? 'to ' : 'by '
	secondPhrase = if option == 0 'to ' else 'past '

	hour = randint(1, 12);
			  
    # numbers = [randint(0, 9) for _ in range(3)]

    question = firstPhrase + secondPhrase + hour

    round_msg = render_template('round', question=question)
